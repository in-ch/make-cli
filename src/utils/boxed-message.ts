import colorMap, { Color } from "@/src/utils/color-map";
...
export default function boxedMessage({
  ...
  const maxLength = messages.reduce(
  ...
  const borderTop = `${borderColorCode}${ltEdge}${"─".repeat(
    boxWidth + 2
  )}${rtEdge}${defaultCode}`;
  const borderBottom = `${borderColorCode}${lbEdge}${"─".repeat(
    boxWidth + 2
  )}${rbEdge}${defaultCode}`;
  const emptyLine = `${borderColorCode}│${" ".repeat(
    boxWidth + 2
  )}│${defaultCode}`;
